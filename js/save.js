...
window.Save = (function () {
  ...
  function defaultSave() {
    return {
      ...
      weapons: {},              // { 武将名: { q: 装配品质 0-4, n: 素材数 } }（8 合 1 精炼）
  ...
  function weapons() { return data.weapons || (data.weapons = {}); }
  // 掉落入库：更高品质直接装配（旧装配变为素材 +1）；否则记入素材
  function addWeapon(generalName, q) {
    const ws = weapons();
    const cur = ws[generalName] || { q: 0, n: 0 };
    if (q > cur.q) { cur.n += 1; cur.q = q; }
    else cur.n += 1;
    ws[generalName] = cur;
    save();
    return cur;
  ...
  function refineWeapon(generalName) {
    const ws = weapons();
    const cur = ws[generalName] || { q: 0, n: 0 };
    if (cur.n < CFG.WEAPON_REFINE || cur.q >= CFG.WEAPON_QUALITIES.length - 1) return null;
    cur.n -= CFG.WEAPON_REFINE;
    cur.q += 1;
    ws[generalName] = cur;
    save();
    return cur;
  }
  function weaponOf(generalName) {
    return weapons()[generalName] || { q: 0, n: 0 };
  }
  ...
  return {
    ...
    weapons, addWeapon, refineWeapon, weaponOf,
    recordGame, winRate,
