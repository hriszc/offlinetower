...
window.Engine = (function () {
  ...
  class Unit {
    constructor(gen, level) {
      ...
      this.cd = 0; this.skillCd = 3;          // 技能冷却（秒），冷却结束自动释放（无怒气、无手动操作）
  ...
  class Battle {
    constructor(map, opts) {
      ...
      // 局间道具常驻加成（由 game 开局按存档 items 设置,图鉴 v1.2 §3.2）
      this.permaCd = 1;        // 锦囊妙计: 技能冷却 ×0.85
      this.permaDmgAdd = 0;    // 乱世奸雄: 全队造成伤害 +8%
      this.permaEliteDmg = 0;  // 威震华夏: 对精英/BOSS 伤害 +30%
      this.permaFrq = 0;       // 人中赤兔: 全队攻速 +10%
      this.permaKillFrq = 0;   // 一身是胆: 击杀叠攻速开关（每层 +2%,上限 15 层）
      this.killFrqStacks = 0;
      this.permaSkillDmg = 0;  // 燕人咆哮: 技能伤害 +15%
      this.permaDotExtra = 0;  // 大都督: 灼烧类技能持续时间 +2 秒
      this.permaMonSlow = 0;   // 冢虎之谋: 全场怪物移速 -10%
      // 武器装配（由 game 开局按存档武器设置:{ 武将名: 品质索引 0-4 }）
      this.weaponMap = {};
    ...
    // 武器加成（按武将专属武器品质,图鉴 §4.2：主属性 + 副属性）
    weaponBonus(u) {
      const q = this.weaponMap[u.gen.name];
      if (q === undefined || q === null) return null;
      const w = CFG.WEAPONS.find(x => x.general === u.gen.name);
      const qd = CFG.WEAPON_QUALITIES[q];
      if (!w || !qd) return null;
      const b = { atk: 0, frq: 0, rge: 0 };
      if (w.main === 'atk') b.atk += qd.atk;
      if (w.sub === 'frq') b.frq += qd.frq;
      if (w.sub === 'rge') b.rge += qd.rge;
      return b;
    }
    ...
    spawnMonster(cfg) {
      ...
      // 道具「冢虎之谋」：全场怪物移速 -10%
      if (this.permaMonSlow) m.spd *= (1 - this.permaMonSlow);
    ...
    updateUnit(u, dt) {
      ...
      // 主动技能自动释放：冷却就绪且场上有敌人即释放（无怒气门槛,图鉴 v1.2 §2.1）
      if (u.skillCd <= 0 && this.monsters.some(m => !m.dead) && !this.over) {
        this.castSkill(u);
      }
    }

    effAtk(u) {
      const wb = this.weaponBonus(u);
      return u.baseAtk() * (1 + this.teamBuffs.atkAdd + this.permaDmgAdd + u.buffAtk + (wb ? wb.atk : 0));
    }
    effFrq(u) {
      const wb = this.weaponBonus(u);
      const killFrq = this.permaKillFrq ? Math.min(15, this.killFrqStacks) * 0.02 : 0;
      return u.baseFrq() * (1 + this.teamBuffs.frqAdd + this.permaFrq + u.buffFrq + killFrq + (wb ? wb.frq : 0));
    }
    effRge(u) {
      const wb = this.weaponBonus(u);
      return u.baseRge() + (wb ? wb.rge : 0);
    }

    calcDmg(u, m) {
      const c = this.baseCrit();
      const crit = Math.random() < c.rate ? (1 + c.dmg) : 1;
      // 无职业克制；道具「乱世奸雄」全队造成伤害 +8% / 「威震华夏」对精英/BOSS +30%（图鉴 §3.2）
      const eliteMult = 1 + (this.permaEliteDmg && (m.elite || m.boss) ? this.permaEliteDmg : 0);
      const dmgMult = 1 + this.permaDmgAdd;
      return { dmg: Math.max(1, Math.round(this.effAtk(u) * dmgMult * eliteMult * crit)), crit: crit > 1 };
    }
    ...
    onMonsterKilled(m, attacker) {
      // 道具「一身是胆」：每击杀 1 敌,全队攻速 +2%（上限 15 层）
      if (this.permaKillFrq && this.killFrqStacks < 15) this.killFrqStacks++;
      this.emit('kill', m, attacker);
      if (m.boss) this.emit('bossKill', m);
    }

    /* ---------- 技能（冷却结束自动释放,无怒气/无手动操作） ---------- */
    castSkill(u) {
      if (u.skillCd > 0 || !u.deployed || this.over) return false;
      const s = u.gen.skill;
      const atk = this.effAtk(u);
      const skillMult = 1 + this.permaSkillDmg;   // 道具「燕人咆哮」技能伤害 +15%
      ...
      switch (s.type) {
        case 'bolt': {  // 诸葛亮
          ...
          for (let i = 0; i < 6 && list.length; i++) {
            ...
            const d = t.damage(atk * 1.5 * skillMult, u, this);
        ...
        case 'buff': {  // 曹操 / 刘备（无回血,英雄不承伤）
          if (u.gen.name === '刘备') {
            this.teamBuffs.atkAdd += 0.15; this.teamBuffs.frqAdd += 0.10; this.teamBuffs.until = Math.max(this.teamBuffs.until, this.time + 5);
          } else {
        ...
        case 'single': { // 关羽
          ...
          if (list.length) {
            ...
            const t = list[0];
            const d = t.damage(atk * 4 * skillMult, u, this);
        ...
        case 'dash': {   // 赵云
          ...
          for (const t of line) {
            const d = t.damage(atk * 3 * skillMult, u, this);
        ...
        case 'aoe': {    // 吕布
          ...
          for (const t of list) {
            const d = t.damage(atk * 3.5 * skillMult, u, this);
        ...
        case 'stun': {   // 张飞
          ...
          for (const t of list) {
            t.damage(atk * 1.8 * skillMult, u, this);
            t.stunUntil = this.time + 2;
            this.emit('skillFx', u, 'stun', t);
          }
          break;
        }
        case 'dot': {    // 周瑜（道具「大都督」灼烧 +2 秒）
          for (const t of alive()) {
            t.dot = { dps: atk * 0.8 * skillMult, until: this.time + 4 + this.permaDotExtra };
          }
